...
=item $tr->install_external_function($namespace, $function_name, $function, \%opt)
...
C<%attr> is an optional hashref attribute. Currently supported attribute is C<Context>, which is default to FALSE. 
When this is set, then a context node is always passed to the callback function as the first argument. 
A context node is an C<XML::Xalan::DOM::Node> object. 
...
C<XML::Xalan::DocumentBuilder>(3), C<XML::Xalan::ParsedSource>(3), C<XML::Xalan::DOM>(3).
